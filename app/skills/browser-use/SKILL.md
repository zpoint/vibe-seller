---
name: browser-use
description: "MUST load BEFORE running any browser-use command. This is the only bridge between the agent and the browser — contains all CLI syntax (open, click, hover, scroll, input, state, eval, etc.), wrapper rules, session management, and store-task restrictions. Without this skill, browser-use commands will fail or use wrong syntax."
allowed-tools: Bash(browser-use:*)
---

<!-- VIBE-SELLER CUSTOMIZATIONS: If syncing from upstream browser-use,
     re-apply: (1) this Store Tasks warning banner, (2) the Browser Modes
     section replacement (removed --profile/--cdp-url/--connect examples),
     (3) removed Authenticated Browsing and Connecting to Existing Chrome
     workflow sections, (4) "(blocked in store tasks)" notes in Global
     Options table. See git blame for original changes. -->

> **Store Tasks:** When running inside a store task, `browser-use` is wrapped
> by a per-store script that auto-injects `--session` and `--cdp-url`.
...
browser-use --session <name> open <url>        # Target a named session
```

> **Store tasks only:** `--profile`, `--cdp-url`, and `--connect` are blocked
> by the store wrapper. The wrapper auto-injects the correct session and CDP
> URL. Use `--session <slug>-aux` for non-seller-center browsing.
...
| `--json` | Output as JSON |
| `--profile [NAME]` | Use real Chrome profile (**blocked in store tasks**) |
| `--connect` | Auto-discover running Chrome (**blocked in store tasks**) |
| `--cdp-url <url>` | Connect via CDP URL (**blocked in store tasks**) |
| `--mcp` | Run as MCP server (**blocked in store tasks**) |
