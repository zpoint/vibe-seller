...
shadow root** (light-DOM `input[type=file]` count is 0). Drive it via
the wrapper's indexed `state` (`upload <index> <path>`), then click
**Submit products**. Amazon processes asynchronously; the batch row on
