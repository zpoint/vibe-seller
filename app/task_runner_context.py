...
def build_store_context(
    ...
    if backend == 'ziniao':
        ctx = (
            ...
            f'\n## BROWSER\n'
            f'Run `browser-use` CLI commands via Bash for '
            f'browser automation.\n'
            f'IMPORTANT: Before using any browser-use commands, '
            f'load the browser-use skill first (`/browser-use`). '
            f'Do NOT guess or hallucinate CLI arguments — the '
            f'skill documents the exact syntax.\n'
            ...
            f'store. NEVER extract cookies using '
            f'`browser-use cookies get` or '
            f'`browser-use cookies export` '
            f'and use them with curl, requests, Python http '
            f'clients, or any tool outside browser-use. This '
            f'bypasses IP isolation and will trigger platform '
            f'security alerts. ALL authenticated page access '
            f'MUST go through browser-use commands only.\n'
            f'\n## DUAL BROWSER SYSTEM\n'
    ...
    if backend == 'ziniao':
        ctx += (
            ...
            'IMPORTANT: `browser-use state` does NOT show '
            'auto-filled input values — fields appear empty '
            'even when filled.\n'
            'Use `browser-use get value <index>` to check '
            'whether a field has been filled (passwords are '
            'redacted automatically).\n\n'
            'Login flow:\n'
            '1. Open login URL, `sleep 3` for auto-fill\n'
            '2. `browser-use state` to find input indices\n'
            '3. `browser-use get value <index>` on password/'
            'OTP fields — non-empty means filled\n'
            '4. If filled, click Sign-in/Submit directly '
            '— do NOT re-type credentials\n'
            '5. For OTP/2FA: `sleep 5` for Ziniao to fetch '
            'the code, then `browser-use get value`, then '
            'click Submit\n'
            '6. Only ask the user if `get value` confirms '
...
async def build_system_context(task: Task) -> str:
    ...
    lines.append(
        ...
        '\n'
        '**Picking the right tool when the task did not '
        'finish.** `vibe_seller_set_task_result` declares '
        '**success**: the finalizer marks the task COMPLETED and '
        'the user sees a green status. If the primary objective '
        'could not be completed for any reason (infra failure, '
        'blocker on the page, missing prerequisite, you ran out '
        'of options), you MUST call `vibe_seller_set_task_error` '
        '— calling only `vibe_seller_set_task_result` with prose '
        'like "could not finish" will be marked COMPLETED and is '
        'misleading. If you have partial useful output to show, '
        'call BOTH: `vibe_seller_set_task_result(partial_summary)` '
        'for what was gathered, then '
        '`vibe_seller_set_task_error(reason it did not finish)` '
        'to land the task in FAILED with the partial summary '
