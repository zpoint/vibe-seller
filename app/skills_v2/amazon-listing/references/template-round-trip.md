...
> in the card's FOOTER**; `click_at_xy` it (screenshot to find its
> coords). That navigates to `/product-search/bulk/generate/add-product`,
> the product-type search page.
>
...
> list. `click_at_xy` the **search icon** (`kat-icon name=search`) at the
> right of the input row to trigger the suggestions, then `click_at_xy`
> the **Select** button on the matched product type.
