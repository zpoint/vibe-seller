...
tab click + a table read + a Customer-Queries read). For a portfolio of
...
- **Use trusted clicks (`click_at_xy`) for Save/Apply/toggle
  actions** — `capture_screenshot()` to find the coordinates, then
  `click_at_xy(x, y)` (a `js("…click()")` is ignored by the React
  handlers). Reserve `js()` for navigation and reading only.
