...
Agent 通过 CDP 驱动紫鸟（或普通 Chrome），跟你手动点页面一样
...
flowchart LR
    U1[你] -.内网.-> S
    U2[同事] -.内网.-> S
    subgraph OfficeHost["办公电脑（Mac 或 Windows，常开）"]
...
（OpenClaw、n8n、影刀）都只能玩普通 Chrome，没一个能驱动紫鸟。
Vibe Seller 就是补这一块。
...
- **天然多店隔离**——每个店铺各有自己的平台、数据、SOP、积累
  下来的经验。店铺 A 跑 Amazon US，店铺 B 跑 Noon AE，店铺 C
...
- 操作系统 —— **macOS** *或* **Windows 11+ 配 WSL2**

  <sub><b>对于 Windows 用户：</b>WSL 和 Windows 侧的浏览器要互通，WSL 必须以 <b>mirrored 网络模式</b> 启动，而 mirrored 只有 Windows 11 及以上才支持。</sub>
- Python 3.11+（缺失时 [`uv`](https://docs.astral.sh/uv/) 自动装）
- Node.js 22+
- 任意一个 LLM API Key：Claude / DeepSeek / Kimi / MiniMax / GLM / Qwen
- 紫鸟（推荐，但可选）——不用紫鸟就走普通 Chrome
...
升级：`vibe-seller upgrade`。卸载：`uv tool uninstall vibe-seller`。
...
- **Mac**：按 ⌘ + 空格，输入「终端」，回车。
- **Windows**：先装 WSL2（搜「Windows WSL2 安装」有大把教程），打开「Ubuntu」。
...
个模式才会开 CDP）。WSL 这边没法直接重启 Windows 上的紫鸟，所以
要在 Windows 侧用启动器：
...
| 任务类型 | 推荐模型 |
|---|---|
| **简单任务** —— 邮件分类、固定流程数据抽取、按字段填表 | MiniMax、DeepSeek、Claude、Kimi 都行，挑便宜的 |
| **复杂任务** —— 探索新平台（没有内置 Skill）、广告优化、库存策略、跨页面长流程 | **建议 DeepSeek、Claude 等 SOTA 模型，别用便宜模型** |
...
行——普通 Chrome 完全够用，只是没有紫鸟的指纹隔离。如果你已经
在用紫鸟做跨境，那直接拿过来就行，Vibe Seller 天然适配你日常的
店铺档案。
...
没开通的话紫鸟 API 会返回 `statusCode: -10003`，Vibe Seller 会把
这个错误抛给你看。

**凭据存在哪**——全部在你本地 `~/.vibe-seller/data/vibe_seller.db`
（SQLite）。没有上传，遥测通道也不带走。

- **密码**：落盘前用 Fernet 加密（密钥从你本地的 `JWT_SECRET` 用
  SHA256 派生），任何 HTTP 接口都不会把密码返回给前端——只在
  Vibe Seller 要调紫鸟 HTTP API 那一刻才在内存里解密。相关代码
  在 `app/utils/crypto.py` 和 `app/routers/ziniao_accounts.py`。
