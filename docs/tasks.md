...
- `waiting`: long-running tasks paused for external events (e.g., marketplace case response, listing approval). Triggered these ways: (1) agent signals via `wait-condition` block in result text, (2) agent exits with incomplete `TodoWrite` items (`_has_incomplete_todos` → `mark_waiting_for_input`, `check_strategy: manual`), (3) agent exits while an `AskUserQuestion` is still outstanding (`park_waiting_for_pending_question` in `app/task_runner.py`) — the question payload is persisted into `wait_condition.pending_question` so the operator can answer later via the UI, or (4) **interactive-Q&A prose follow-up**: the agent called `AskUserQuestion`, got answered, then ended its final segment with PROSE ONLY — no follow-up tool action and no explicit `vibe_seller_set_task_result` (itself a tool_use). That shape is "asking again / awaiting more input", not "done" — but the streaming-prose fallback already populated `task.result`, so the empty-result waiting-parks above (all gated on `not task.result`) miss it and the task would silently COMPLETE mid-conversation. `_finalize_terminal_state` detects it structurally via two session signals — `_asked_user_question` (a question was asked this session) AND NOT `_tool_use_since_answer` (no real, non-reflection tool ran after the last answer) — and calls `park_waiting_for_text_only_response`. It is deliberately NOT a content heuristic: a single-turn task that never asked anything (`_asked_user_question=False`) always completes, and FakeAgent sessions lack these attrs (getattr default False) so workflow finalizer tests are unaffected. System periodically checks email conditions or user manually wakes.
...
- **`claude_backend.py`**: Claude Code CLI subprocess implementation
  ...
  - **Turn lifecycle (process-per-turn model)**: a turn is one CLI process, and the turn ends when the process exits. The CLI never exits on its own while stdin is open (spike-verified vs claude 2.1.215), so closing stdin is the turn terminator — owned by the **quiescence watchdog** (`app/ai/claude_backend_turns.py`): stdin closes only when an accepted result exists for the current turn, the review/exec gates pass, no tracked async work is pending, no AskUserQuestion is parked, and the stream has been idle for the linger window (`VIBE_TURN_LINGER_S` when async work was launched / `VIBE_TURN_LINGER_QUIET_S` otherwise; `0` = close at the result event, the current default until the flip). **Async work = async subagents (Agent tool) AND background shell commands** (Bash/PowerShell auto-backgrounded or `run_in_background`); both register in `_async_agents` and clear on their `<task-notification>`, so a turn does not end while a `find`/export it launched is still running. Backstops: `VIBE_TURN_HARD_IDLE_S` closes on total stream silence regardless of holds, and a post-close kill escalation `_force_kill()`s a process that stays alive past the grace (bounds the GLM stall-after-result wedge the old design left unbounded). A runaway background command (never completes → force-closed while running → killed with no completion record) would poison the next `--resume` with a reconciliation notification, so a follow-up starts a **fresh session** when the prior one has an orphaned background task (`session_has_orphaned_bg_task`). Every ACCEPTED execute-phase result event is a **turn boundary**: it emits its own `role='result'` card (a process hosts many turns — redrives, task-notification continuations, injected follow-ups) and the LAST non-empty one is the deliverable (`_result_text`, last-wins; explicit `set_task_result` still takes precedence). `_last_result_is_error` is per-result (assigned, not sticky) and folded into `_is_error_result` at exit so a recovered turn ships COMPLETED while forced errors (circuit breaker, rc!=0) stay FAILED. During planning stdin always stays open (multi-turn feedback); the plan-skip close (result with no plan saved) is immediate to avoid a deadlock.
...
- **What starts a turn**: task creation / plan approval (first turn of
  ...
  (an async subagent OR background shell command finished and the main
  agent resumes), or a follow-up message injected into the live process.
- **What ends a turn**: an accepted result event (gates pass, no async
  work — subagents or background shells — pending) emits the turn's
  `role='result'` card. **What
