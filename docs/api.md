...
| POST | `/api/tasks/{task_id}/ad-declaration` | Declare what THIS PHASE of an ad task is for, before any browser work (agent-only via MCP `vibe_seller_declare_ad_task`). Body: `{kind, scope}` where kind ∈ audit/create/execute/investigate — there is no `edit`; "change these three bids" is a one-campaign `audit`. `scope` = `{combos[], campaigns[], products[]}`; **absent `combos` means every marketplace the store sells on**. Append-only: a second declaration is 409 unless a NEW user message has arrived since the last one, so only the person typing can open a phase. **Two exceptions, both ratchets** (`ad_declaration.supersedes`) — within a turn you may narrow at the same kind (recording campaign ids only enumeration could reveal), or correct `investigate` → `audit` at the same-or-narrower reach. The latter is legal because it *adds* obligation (coverage + a console); `audit` → `investigate` and every other pair stay 409, as does any widening. On accept, mirrored to `AD_TASK.json` in the task workspace for the stop gates (the row stays authoritative). |
| GET | `/api/tasks/{task_id}/ad-declarations` | Every declaration this task has made, oldest first. The frontend binds each result item to the declaration in force when it landed: `kind == 'audit'` opens the review console, and its `scope` bounds which campaigns are actionable. |
