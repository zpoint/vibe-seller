...
| POST | `/api/tasks/{task_id}/ad-declaration` | Declare what THIS PHASE of an ad task is for, before any browser work (agent-only via MCP `vibe_seller_declare_ad_task`). Body: `{kind, scope}` where kind ∈ audit/create/execute/investigate — there is no `edit`; "change these three bids" is a one-campaign `audit`. `scope` = `{combos[], campaigns[], products[]}`; **absent `combos` means every marketplace the store sells on**. Append-only: a second declaration is 409 unless a NEW user message has arrived since the last one, so only the person typing can open a phase. On accept, mirrored to `AD_TASK.json` in the task workspace for the stop gates (the row stays authoritative). |
