...
**Dual browser (Ziniao)**: Ziniao stores get dual-session support in their wrapper — main session (`{slug}-{task[:8]}`) routes to Ziniao via CDP proxy, aux session (`{slug}-aux`) uses Chrome directly. No separate backend instance for aux — browser-use manages Chrome sessions natively.
